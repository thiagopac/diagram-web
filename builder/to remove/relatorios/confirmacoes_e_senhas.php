...
#PUXA OS DADOS DO BANCO DO CLIENTE
	if (($ID_CLIENTE > 1) && ($MSG == ''))
		...
		$SQL = "select  distinct DATE_FORMAT(date(LISTA_ALUNO.dtconfirmacao),'%d/%m/%Y')  din,
				...
				where
				  ( '' = '$PESQUISA' or
					...
					LISTA_ALUNO.idtipo = $PESQUISA_INTERFACE)
				  and LISTA_ALUNO.idtipo is not null and LISTA_ALUNO.idtipo in (1,2)
				  and LISTA_ALUNO.idaluno = ALUNO_UNICURSO.idaluno
				  and UNIDADE_CURSO.id = ALUNO_UNICURSO.idunicurso
				  #and UNIDADE_CURSO.idunidade in (110,111,112) #Cada cliente tera seus IDs de UNIDADE DE ENSINO
				  and LISTA_ALUNO.idaluno = ALUNO.id
				  and LISTA_ALUNO.dtconfirmacao >= curdate()-interval 10 day
		...
		/*$SQL = "select DATE_FORMAT(date(dtcriacao),'%d/%m/%Y') din,status ,statusdesc ,count(*) quantidade
				from RESPOSTA_ROBO
				where
				('' = '$PESQUISA' or status = '$PESQUISA' or statusdesc like '%$PESQUISA%')
				and dtcriacao between '$DAT_INICIO 00:00:00' and '$DAT_FIM 23:59:59'
		...
		<div class="page-content">
			...
			<div class="row">
				<div class="col-md-12">
					...
					<h3 class="page-title">
					Confirmações e Senhas <small></small>
					</h3>
